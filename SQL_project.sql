...
(9,6, 'GL Managment')
 ...
 SELECT FIRST_NAME,ISNULL(PROJECT_NAME,'No Project Assigned')FROM EMPLOYEE_DETAIL_TABLE 
    LEFT OUTER JOIN
    PRODUCT_DETAIL_TABLE  
    ON
 ...
 SELECT EMPLOYEE_ID, FIRST_NAME, PROJECT_NAME FROM EMPLOYEE_DETAIL_TABLE 
    ...
    WHERE EMPLOYEE_ID
    IN (SELECT EmployeeDetail_ID FROM PRODUCT_DETAIL_TABLE
    GROUP BY EmployeeDetail_ID HAVING COUNT(*) >1 )
